...
export interface DocumentFrontmatter {
  ...
  date: string;          // ex: "A.R. 2187.04.14"
  issued_by: string;
  reference?: string;    // ex: "DEC/0421-NA"
  summary?: string;
  subject?: string;      // for memo
  to?: string;           // for memo
  from?: string;         // for memo
  subject_name?: string; // for dossier
  status?: string;       // for dossier / incident
  incident_code?: string;
  location?: string;
  channel?: string;      // for transmission
...
const registry: Record<string, ArchiveDocument> = {};
...
for (const path in modules) {
  ...
  const slug =
    fm.slug ?? path.split("/").pop()!.replace(/\.mdx$/, "");
  registry[slug] = {
    frontmatter: { ...fm, slug },
    Content: mod.default,
  };
...
export function getBatchItems(
  ...
  if (!fm.items) return [];
  return fm.items.map((it) => ({ ...it, doc: registry[it.slug] }));
}
