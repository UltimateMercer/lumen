/**
 * ============================================================================
 *  Dossier — Standalone single-file component
 * ============================================================================
 *
 ...
 *  1) Install peer deps:
 *       npm i framer-motion
 *     React 18+ and Tailwind CSS are assumed.
 *
 *  2) Drop this single file anywhere in your project, e.g.
 *       src/components/Dossier.tsx
 *
 *  3) Add the CSS tokens below to your global stylesheet (globals.css /
 *     index.css / styles.css). Tweak any value to re-skin the component.
 *
 *  ----------------------------------------------------------------------------
 *  REQUIRED CSS TOKENS (paste into your global CSS)
 *  ----------------------------------------------------------------------------
 *
 *  :root {
 *    --dossier-radius: 2px;
 ...
 *    --paper-ink:  oklch(0.2 0.025 60);
 *    --paper-edge: color-mix(in oklab, var(--paper-ink) 45%, transparent);
 *
 *    --ink-red:    oklch(0.5 0.22 28);
 *    --neon-red:   oklch(0.62 0.27 25);
 *    --neon-amber: oklch(0.78 0.18 70);
 *    --neon-cyan:  oklch(0.86 0.16 200);
 *
 *    --hud-grid:   color-mix(in oklab, var(--neon-red) 28%, transparent);
 *
 *    --glass-bg:     color-mix(in oklab, var(--paper) 28%, transparent);
 *    --glass-border: color-mix(in oklab, var(--paper-ink) 55%, transparent);
 *    --glass-blur:   16px;
 *
 *    --carbon-bg:   oklch(0.18 0.02 250);
 *    --carbon-ink:  oklch(0.92 0.02 90);
 *    --carbon-edge: color-mix(in oklab, var(--neon-red) 55%, transparent);
 *
 *    --font-mono: "JetBrains Mono", ui-monospace, monospace;
 *
 *    --shadow-folder:
 *      0 1px 0 color-mix(in oklab, var(--paper-ink) 25%, transparent),
 *      0 18px 40px -18px color-mix(in oklab, var(--neon-red) 45%, transparent),
 *      0 30px 60px -30px rgba(0, 0, 0, 0.75);
 *    --shadow-folder-open:
 *      0 0 0 2px color-mix(in oklab, var(--neon-red) 35%, transparent),
 *      0 30px 80px -20px rgba(0, 0, 0, 0.85);
 *  }
 *
 *  Older browsers without oklch() can use plain hex/rgb — only the appearance
 *  changes; the component logic does not depend on a specific color space.
 *
 *  ----------------------------------------------------------------------------
 *  USAGE
 *  ----------------------------------------------------------------------------
 *
 *  // 1) Uncontrolled, with inner payload
 *  <Dossier title="OPERATION NIGHTFALL" caseId="X-7741">
 *    <article>Full briefing here…</article>
 *  </Dossier>
 *
 *  // 2) Controlled
 *  const [open, setOpen] = useState(false);
 *  <Dossier open={open} onOpenChange={setOpen} trigger="manual">…</Dossier>
 *
 *  // 3) Standalone cover (decorative). Click still opens an empty payload;
 *  //    the user dismisses with the [ESC] button or the Esc key.
 *  <Dossier title="CLASSIFIED" layout="minimal" surface="carbon" />
 *
 *  ----------------------------------------------------------------------------
 *  PROPS (defaults in parentheses)
 *  ----------------------------------------------------------------------------
 *
 *   title          ("UNTITLED FILE")  Big cover title.
 *   caseId         (auto e.g. "X-7741")  Identifier shown in metadata + barcode.
 *   date           (today, ISO)       Date string.
 *   classification ("TOP SECRET")     Clearance line.
 *   stamps         (["CLASSIFIED"])   Array of stamp labels or {label, shape,
 *                                     rotate, ring} objects.
 *   showBarcode    (true)             Procedural pseudo-barcode from caseId.
 *   showHud        (true)             Shorthand: toggles grid + scanlines + corners.
 *   showScanlines  (—)                Overrides showHud for horizontal scanlines.
 *   showGrid       (—)                Overrides showHud for background grid.
 *   showCorners    (—)                Overrides showHud for corner brackets.
 *   crest          (false)            true | { label, ring, ornate } — agency seal.
 *   layout         ("default")        "default" | "crest-hero" | "minimal" | "field-report".
 *   surface        ("paper")          "paper" | "glass" | "carbon".
 *   paperTexture   ("noise")          "noise" | "grain" | "fiber".
 *   briefing       (—)                Short paragraph used by "field-report" layout.
 *   animation      ("flip3d")         flip3d | slide | glitch | combo | scale-rise |
 *                                     peel | shred | iris | double-cover.
 *   trigger        ("click")          "click" | "hover" | "manual".
 *   open / onOpenChange (—)           Controlled open state.
 *   dismissible    (true)             Show [ESC] CLOSE + react to Esc key.
 *   children       (—)                Inner content. Optional — works standalone.
 *   className      ("")               Extra classes on the outer wrapper.
 *   aspect         ("portrait")       "portrait" (3/4) | "landscape" (4/3) | "square".
 *
 *  ----------------------------------------------------------------------------
 *  EXTENDING
 *  ----------------------------------------------------------------------------
 *
 *  Add a new LAYOUT:
 *    1. Add the variant name to `DossierLayout`.
 *    2. Implement a `CoverMyLayout(props: CoverLayoutProps)` function below
 *       (copy any of the existing `CoverXxx`).
 *    3. Add a `case "my-layout": return <CoverMyLayout {...rest} />;` to the
 *       switch inside `DossierCover`.
 *
 *  Add a new ANIMATION:
 *    1. Add the name to `DossierAnimation`.
 *    2. Add a `Variants` entry to `coverVariants` (closed/open states).
 *    3. If it needs custom DOM (like `shred` or `double-cover`), branch in the
 *       main `Dossier` render the same way those two do.
 *
 *  Add a new SURFACE:
 *    1. Add the name to `DossierSurface`.
 *    2. Add a branch to `surfaceStyle()` returning background/border/color/shadow.
 *    3. Optionally adjust `inkVar()` for divider/ink color.
 *
 *  Add a new PAPER TEXTURE:
 *    1. Add the name to `DossierPaperTexture`.
 *    2. Add a `{variant === "my-texture" && …}` block inside `PaperTexture`.
 *
 *  Re-skin quickly:
 *    - Change `--neon-red` → whole HUD/accents shift.
 *    - Change `--paper` + `--paper-ink` → all paper surfaces shift.
 *    - Change `--dossier-radius` → corner roundness.
 *    - Change `--font-mono` → all metadata/typography.
 *
 *  ----------------------------------------------------------------------------
 *  RESPONSIVENESS
 *  ----------------------------------------------------------------------------
 *  Padding uses Tailwind breakpoints (`p-4 sm:p-5 md:p-6`).
 *  Typography uses `clamp()` so titles scale with viewport.
 *  Crests / stamps / notches use `clamp()` for fluid sizing.
 *  Layouts collapse to single column below the `sm` breakpoint where relevant.
 *
 *  ----------------------------------------------------------------------------
 *  ACCESSIBILITY
 *  ----------------------------------------------------------------------------
 *  - Trigger is a real <button> with `aria-expanded` + `aria-controls`.
 *  - Content panel is focused when revealed.
 *  - Esc key closes when `dismissible` (default).
 *  - `useReducedMotion()` falls back to the `slide` preset.
 *  - All decorative SVG / overlays are `aria-hidden`.
 *
 *  ============================================================================
 */
...
function resolveCrest(crest: DossierProps["crest"]): false | DossierCrest {
...
export function Dossier({
  ...
  // Always dismissible by default — even standalone, the user needs a way back to the cover.
