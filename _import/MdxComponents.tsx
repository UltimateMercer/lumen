...
/**
 * Componentes injetados dentro do MDX dos documentos.
 * Cada um carrega significado narrativo dentro do universo MINCONT-OS.
 */
...
/**
 * Carimbo governamental estilizado.
 * Shape padrão circular, com leve rotação determinística e textura de tinta.
 */
...
export function Stamp({
  ...
  return (
    <span
      ...
      style={{ ["--stamp-rotate" as string]: `${r}deg` }}
    >
      <span>{children}</span>
      {subtitle && (
        <>
          <span className="stamp-divider" />
          <span className="text-[0.55rem] opacity-80">{subtitle}</span>
        </>
      )}
    </span>
  );
...
export function Pullquote({ children, by }: { children: ReactNode; by?: string }) {
  return (
    <blockquote className="my-4 break-inside-avoid border-y-2 border-paper-foreground/50 py-3 text-center font-display text-lg italic leading-snug text-paper-foreground">
      “{children}”
      {by && <footer className="mt-1 text-[10px] not-italic uppercase tracking-[0.3em] text-paper-muted">— {by}</footer>}
    </blockquote>
  );
}
...
export function Exchange({
  ...
  const toneClass =
    tone === "tense"
      ? "text-stamp-red"
      : tone === "redacted"
        ? "text-paper-muted italic"
        : "text-paper-foreground";
  const accent =
    tone === "tense"
      ? "before:bg-stamp-red"
      : tone === "redacted"
        ? "before:bg-paper-muted/50"
        : "before:bg-paper-foreground/20";
  return (
    <div
      className={cn(
        "exchange-row relative grid gap-x-4 gap-y-1 border-b border-dashed border-paper-foreground/15 py-2.5 pl-3 text-sm md:grid-cols-[6ch_14ch_1fr]",
    ...
    >
      ...
      <span className="font-mono text-[11px] font-bold uppercase tracking-[0.15em] text-paper-foreground">
        {speaker}
      </span>
...
export function Note({
  ...
  kind?: "pause" | "inaudible" | "off-record" | "action";
  children?: ReactNode;
}) {
  const label =
    kind === "pause"
  ...
  return (
    <div className="my-3 flex justify-center">
      <span className="border border-dashed border-paper-foreground/30 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.3em] text-paper-muted">
        [ {label}
        {children ? <> · <span className="text-paper-foreground/70">{children}</span></> : null}
        {" "}]
      </span>
    </div>
  );
...
export function Msg({
  ...
  // Pré-coleta determinística das notas das FlagPhrases descendentes.
  // Um contador mutável é fornecido por contexto e incrementado por cada
  // FlagPhrase renderizada, garantindo numeração coerente com a lista.
  ...
  return (
    <div className={cn("thread-msg", side === "right" ? "thread-msg--right" : "thread-msg--left")}>
      <div className="thread-msg-meta">
        ...
        {device ? <span className="opacity-60"> · {device}</span> : null}
        {ts ? <span className="opacity-60"> · {ts}</span> : null}
        {flagged ? <span className="ml-2 text-stamp-red">▲ alerta</span> : null}
      </div>
      <MsgFootnoteContext.Provider value={counter}>
        <div className={cn("thread-msg-bubble", flagged && "thread-msg-bubble--flag")}>
          {children}
        </div>
        {notes.length > 0 && (
          <ol className="thread-msg-notes">
            {notes.map((n, i) => (
              <li key={i}>
                <span className="thread-msg-notes-ref">{toSuperscript(i + 1)}</span>
                <span>{n}</span>
              </li>
            ))}
          </ol>
...
export function FlagPhrase({
  ...
  return (
    <span className="flag-phrase">
      {children}
      {index !== null ? (
        <sup className="flag-phrase-ref">{toSuperscript(index)}</sup>
      ) : null}
    </span>
  );
}
...
export function Warning({ children }: { children: ReactNode }) {
  return (
    <div className="my-4 border-l-4 border-stamp-red bg-stamp-red/[0.06] px-4 py-3 text-sm text-paper-foreground">
      <div className="mb-1 font-mono text-[10px] font-bold uppercase tracking-[0.3em] text-stamp-red">
        ⚠ advertência do conselho
      </div>
      {children}
    </div>
  );
  ...
  return (
    <ol className="my-4 grid gap-2">
      {items.map((it, i) => (
        <li
          key={i}
          className="grid grid-cols-[3ch_1fr] items-baseline gap-3 border border-paper-foreground/25 px-3 py-2 text-sm text-paper-foreground"
        >
          <span className="font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-stamp-red">
...
export function Phase({
  ...
  return (
    <div className="my-3 grid grid-cols-[5ch_1fr] gap-3 border-l-2 border-paper-foreground/40 py-1.5 pl-3">
      <div className="font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-paper-muted">
        E.{String(n).padStart(2, "0")}
      </div>
      <div>
        <div className="font-display text-sm font-bold uppercase tracking-wider text-paper-foreground">
          {name}
        </div>
    ...
    <ul className="my-4 grid gap-1.5 border border-paper-foreground/30 bg-paper-foreground/[0.03] p-3 text-sm">
      {items.map((it, i) => (
        <li key={i} className="grid grid-cols-[2ch_1fr] gap-2 text-paper-foreground">
          <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-stamp-red">
            ▸
          </span>
          <span>{it}</span>
        </li>
...
export function AssetEntry({
  ...
  age?: string | number;
  intake?: string;
  status?: "ativo" | "embedded" | "descontinuado" | "comprometido";
  children?: ReactNode;
}) {
  const statusTone =
  ...
  return (
    <div className="asset-row my-2 grid grid-cols-[12ch_4ch_1fr_auto] items-baseline gap-3 border-b border-dashed border-paper-foreground/25 py-2 text-sm">
      <span className="font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-paper-foreground">
        {codename}
      </span>
      <span className="font-mono text-[10px] text-paper-muted">{age ?? "—"}</span>
      <span className="text-paper-foreground/90">
        {children ?? <Redacted length={18} />}
        {intake ? <span className="ml-2 text-[10px] uppercase tracking-widest text-paper-muted">ingresso {intake}</span> : null}
      </span>
      <span className={cn("font-mono text-[10px] font-bold uppercase tracking-[0.25em]", statusTone)}>
        {status}
      </span>
    </div>
  );
  ...
  return (
    <div className="safeguard-block my-4 grid grid-cols-[auto_1fr] gap-3 border border-dashed border-stamp-red/60 bg-stamp-red/[0.04] p-3 text-sm text-paper-foreground">
      <div className="flex h-full items-start">
        <span className="rotate-180 font-mono text-[9px] font-bold uppercase tracking-[0.3em] text-stamp-red [writing-mode:vertical-rl]">
          salvaguarda {code ?? ""}
        </span>
      </div>
      <div>{children}</div>
...
export function Objective({ children }: { children: ReactNode }) {
  return (
    <div className="my-4 border-l-4 border-paper-foreground/70 bg-paper-foreground/[0.04] px-4 py-3 text-sm text-paper-foreground">
      <div className="mb-1 font-mono text-[10px] font-bold uppercase tracking-[0.3em] text-paper-muted">
        ◆ objetivo declarado
      </div>
      {children}
    </div>
...
/* === Section + ProjectTOC (projetos classificados navegáveis) === */
...
export function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="grid grid-cols-[160px_1fr] gap-3 border-b border-paper-muted/30 py-2 text-sm">
      <div className="text-xs font-bold uppercase tracking-wider text-paper-muted">
        {label}
      </div>
      <div className="text-paper-foreground">{children}</div>
    </div>
...
export function Signature({ name, role }: { name: string; role: string }) {
  return (
    <div className="mt-10 max-w-xs">
      <div className="border-b border-paper-foreground/60 pb-1 font-display text-2xl italic text-paper-foreground">
        {name}
      </div>
      <div className="mt-1 text-xs uppercase tracking-wider text-paper-muted">
        {role}
      </div>
    </div>
  );
...
export function Transmission({ children }: { children: ReactNode }) {
  return (
    <pre className="my-4 whitespace-pre-wrap border-l-2 border-amber-crt bg-chrome/60 p-4 font-mono text-sm leading-relaxed text-amber-crt crt-glow">
      {children}
    </pre>
  );
}
...
export function CensorEntry({
  ...
  return (
    <div className="my-3 grid grid-cols-[40px_1fr_120px] gap-3 border-b border-paper-foreground/20 py-2 text-sm">
      ...
      <div>
        <div className="font-bold uppercase tracking-wider text-paper-foreground">
          {title}
        </div>
        {author && (
          <div className="text-[10px] uppercase tracking-widest text-paper-muted">
            atribuído a {author}
          </div>
        )}
        {children && <div className="mt-1 text-xs opacity-80">{children}</div>}
      </div>
      <div className="text-right text-[10px] uppercase tracking-widest text-stamp-red">
        {clause}
      </div>
    </div>
  );
...
export function RenderMdx({ Content }: { Content: React.ComponentType }) {
  const MdxContent = Content as React.ComponentType<{
